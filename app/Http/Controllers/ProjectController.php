...
{   
    ...
    */
    public function index(Request $request){
        // dd(Crofun::setFirstCodeProject(4));
    ...
    */
    ...
    public function create(Request $request){
      ...
      if ($credit_expect){
       ...
       if ($request->isMethod('post')) {
            ...
            /*プロジェクト名重複チェック*/
            //  if($this->checkprojectCreate($request)){
                       
            //        return view('project.create',['message' => trans('message.project_name_unique'),'customer_id' => $customer_id,'company_id' => $company_id,'transaction'=>$transaction ,'credit_expect'=>$credit_expect]);
            ...
            if($project->save()){ // if save success
             ...
             $char_code = array('Z','Y','X','U','T','R','Q','P');
             $max_code  = str_replace($char_code,'',$project->project_code);
            
             $company   = Company_MST::where('id',$project->company_id)->first();
             // $get_max_code = $this->save_max_code($company->project_max_code,$max_code);
    ...
    // public function  save_max_code($saved_code,$pr_code){
        
    //     if($saved_code >= 8000){

    //         return 8000 + $pr_code+1;
    //      }
                 
    //      if($saved_code >= 7000){

    //         return 7000 + $pr_code+1;
    //      }

    //      if($saved_code >= 6000){
            
    //         return 6000 + $pr_code+1;
    //      }

    //     if($saved_code >= 5000){

    //         return 5000 + $pr_code+1;
    //      }

    //     if($saved_code >= 4000){

    //         return 4000 + $pr_code+1;
    //      }

    //     if($saved_code >= 3000){

    //         return 3000 + $pr_code+1;
    //      }

    //     if($saved_code >= 2000){

    //         return 2000 + $pr_code+1;
    //      }

    //     if($saved_code >= 1000){

    //         return 1000 + $pr_code+1;
    //      }
      ...
      //開くプロジェクトの取得
       ...
       //  $transaction =  $transaction_expect + $transaction_shot;
        ...
        if ($request->isMethod('post')) {
            ...
            /*プロジェクト名重複チェック*/
            //  if($this->checkprojectEdit($request,$project)){
                       
            //        return view('project.edit',['message' => trans('message.project_name_unique'),'project' => $project,'transaction'=>$transaction ,'credit_expect'=>$credit_expect]);
        ...
        // if(($this->authorize('view', $project) == true)&&($request->show != null)){
