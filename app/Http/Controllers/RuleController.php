...
class RuleController extends Controller
{
    ...
    public function edit(Request $request){
        ...
        // $menus = Menu::orderBy('id', 'asc')->where('dis_sort', '<>', '1')->orwhereNull('dis_sort')->get();
        $menus = Menu::orderBy('id', 'asc')->get();
        ...
        if ($request->isMethod('post')) {
            ...
            try {
             ...
             //  Crofun::log_update($rule,config('constant.RULE'),config('constant.RULE_EDIT'),$rule->company_id,$rule->rule,null);
              ...
              Crofun::log_create(Auth::user()->id,$rule->id,config('constant.RULE_RULE'),config('constant.operation_UPDATE'),config('constant.RULE_EDIT'),$rule->company_id,$rule->rule,null,json_encode($rule),$old_date);
                ...
                $res = Rule_action::where('rule_id',$request->rule_id)->delete();
                ...
                DB::commit();
                $new_date = Rule_action::where('rule_id',$request->rule_id)->get();
              //UPADTEのログ
