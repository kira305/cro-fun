...
class UserController extends Controller
{
        ...
        if ($request->isMethod('post')) {
          ...
          if(Auth::user()->rule()->superuser_user == 1){
              ...
              $users = $this->user_service->searchBySuperRule(
                                                         $request->company_id,$request->headquarter_id,
                                                         $request->department_id,$request->group_id,
                                                         $request->usr_code,$request->usr_name,
                                                         $request->position_id,
                                                         $request->rule_id
                                                       ); 
          }else {
    ...
    public function concurrent_create(Request $request){
          ...
          if ($request->isMethod('post')) {
            ...
            /*
            if($this->checkCodeIsExistWhenConcurrentCreate($request)){
                          
                  $validator->errors()->add('unique',trans('validation.data_doulble'));
...
/*
    public function checkCodeIsExistWhenConcurrentCreate(Request $request){
          dd($request);
            
       $user = Concurrently::where('usr_code',$request->usr_code)->where('company_id',$request->company_id)->where('headquarter_id',$request->headquarter_id)->where('department_id',$request->department_id)->where('group_id',$request->group_id)->first();
       ...
       if($user){
        ...
        public function concurrent_edit(Request $request){
          ...
          if ($request->isMethod('post')) {
            ...
            /*
            if($this->checkCodeIsExistWhenConcurrentEdit($request)){
                          
                  $validator->errors()->add('unique',trans('validation.code_unique'));

                  $errors = $validator->errors();
                   
                  return view('user.concurrent_edit',["errors"     => $errors,
    ...
    /*
    public function checkCodeIsExistWhenConcurrentEdit(Request $request){
          

       $concurrent = Concurrently::where('usr_code',$request->usr_code)->where('company_id',$request->company_id)->where('headquarter_id',$request->headquarter_id)->where('department_id',$request->department_id)->where('group_id',$request->group_id)->first();
       if($concurrent != null){
           
                 if($concurrent->id != $request->concurrent_id){
    ...
    public function changePass(Request $request){
              ...
              if ($request->isMethod('post')) {
                 ...
                 $validator = Validator::make($request->all(),[
                      ...
                      'now_pass'      => 'required',
                      'new_pass_1'    => 'required|min:6|regex:/^(?=.*[A-Z])(?=.*?\d)(?=.*?[a-z])^[a-zA-Z0-9!#$%&()*+,.:;=?@\[\]^_{}-]+$/',
                      'new_pass_2'    => 'required|min:6|regex:/^(?=.*[A-Z])(?=.*?\d)(?=.*?[a-zA-Z])^[a-zA-Z0-9!#$%&()*+,.:;=?@\[\]^_{}-]+$/',
                      ...
                      'new_pass_1.min'            => trans('validation.password_min_lenght'),
                      'new_pass_1.regex'          => trans('validation.password_format'),
                      'new_pass_2.required'       => trans('validation.password_new_2'),
                      'new_pass_2.min'            => trans('validation.password_min_lenght'),
