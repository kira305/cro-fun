...
{    /**
	...
	public function index(Request $request){
		...
		// 	'pw2'   => 'required',
		// 	'pw3'   => 'required',
  //           'pw2'   => 'required|min:6|regex:/^(?=.*[a-z])(?=.*[A-Z])[a-zA-Z]{6,}$/u',
  //           'pw3'   => 'required|min:6|regex:/^(?=.*[a-z])(?=.*[A-Z])[a-zA-Z]{6,}$/u',
		// ],[
  ...
  //           'pw3.regex'     => trans('validation.password_format'),
        ...
        $validator = Validator::make($request->all(),[
			...
			'pw2'   => 'required',
			'pw3'   => 'required',
            'pw2'   => 'required|min:6|regex:/^(?=.*[A-Z])(?=.*?\d)(?=.*?[a-z])^[a-zA-Z0-9!#$%&()*+,.:;=?@\[\]^_{}-]+$/',
            'pw3'   => 'required|min:6|regex:/^(?=.*[A-Z])(?=.*?\d)(?=.*?[a-z])^[a-zA-Z0-9!#$%&()*+,.:;=?@\[\]^_{}-]+$/',
		],[
			'pw1.required'  => trans('auth.password'),
			'pw2.required'  => trans('auth.password'),
			'pw3.required'  => trans('auth.password'),
